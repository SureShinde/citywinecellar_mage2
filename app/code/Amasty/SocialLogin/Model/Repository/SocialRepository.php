...
class SocialRepository implements SocialRepositoryInterface
{
    ...
    /**
     * contant for attribute code
     *
     * @see createCustomer()
     */
    const DOB_ATTR_CODE = 'dob';
    ...
    protected function importDobData(\Magento\Customer\Api\Data\CustomerInterface $customer, array $user)
    {
        if (isset($user['birthDay']) && $user['birthDay']
            ...
            $dobAttribute = $this->getDobAttribute();
            if ($dobAttribute && $dobAttribute->getIsRequired()) {
    ...
    protected function importGenderData(\Magento\Customer\Api\Data\CustomerInterface $customer, array $user)
    {
        if (isset($user['gender']) && $user['gender']) {
            ...
            $gernderAttribute = $this->getGenderAttribute();
            if ($gernderAttribute && $gernderAttribute->getIsRequired()) {
                $customer->setGender(self::GENDER_NOT_SPECIFIED);
            }
        }
    }
    ...
    {
        try {
            $gernderAttribute = $this->attributeRepository->get(self::CUSTOMER_ENTITY_TYPE, self::GENDER_ATTR_CODE);
        } catch (NoSuchEntityException $exception) {
            $gernderAttribute = null;
        }

        return $gernderAttribute;
    }

    /**
     * @return \Magento\Eav\Api\Data\AttributeInterface|null
     */
    protected function getDobAttribute()
    {
        try {
            $dobAttribute = $this->attributeRepository->get(self::CUSTOMER_ENTITY_TYPE, self::DOB_ATTR_CODE);
        } catch (NoSuchEntityException $exception) {
            $dobAttribute = null;
        }
