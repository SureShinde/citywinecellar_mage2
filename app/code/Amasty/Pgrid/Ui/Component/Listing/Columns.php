...
//        'tax_class_id',
     ...
     * @param \Amasty\Pgrid\Ui\Component\ColumnFactory $columnFactory
     * @param Attribute\Repository $attributeRepository
     * @param Column\InlineEditUpdater $inlineEditUpdater
     * @param \Amasty\Pgrid\Helper\Data $helper
    ...
    protected function _prepareColumns()
    {
        ...
        $bookmarksCols = is_array($config) && is_array($config['current']) && is_array($config['current']['columns']) ?
        ...
        foreach ($this->components as $id => $column) {
            if ($column instanceof \Magento\Ui\Component\Listing\Columns\Column) {
                ...
                $config['ampgrid'] = [
                    ...
                    'filterable' => isset($bookmarksCols[$id]) && isset($bookmarksCols[$id]['ampgrid_filterable']) ?
                        $bookmarksCols[$id]['ampgrid_filterable'] : !empty($config['filter']),
                    'editable' => isset($bookmarksCols[$id]) && isset($bookmarksCols[$id]['ampgrid_editable']) ?
                        $bookmarksCols[$id]['ampgrid_editable'] : false,
                    'title' => isset($bookmarksCols[$id]) && isset($bookmarksCols[$id]['ampgrid_title']) ?
                        $bookmarksCols[$id]['ampgrid_title'] : (isset($config['label']) ? $config['label'] : ''),
                    'visible' => isset($bookmarksCols[$id]) && isset($bookmarksCols[$id]['visible']) ?
                        $bookmarksCols[$id]['visible'] : (isset($config['visible']) ? $config['visible'] : true),
