 ...
 */
namespace Magestore\MigrateData\Model\Order;

/**
 * Class Creditmemo
 * @package Magestore\Webpos\Model\Sales\Order
 */
class CreateOrder extends \Magento\Framework\DataObject
{
...
//        $objectManager->create(\Magento\Sales\Model\Order::class)->loadByIncrementId($last_order_increment_id)
...
//        $results = $conn->query("SHOW TABLE STATUS LIKE '" . $resource->getTableName('sales/order_item') . "'")->fetchAll();
        $results = $conn->query("SELECT item_id from " . $resource->getTableName('sales_order_item') . " ORDER BY item_id DESC LIMIT 1")->fetchAll();
        
        foreach ($results as $data)
            $this->parent_id_flag = $data['item_id'] - 1;
    }
    ...
    {
        try {
            $createdAt = $this->convertDateTime($sales_order_arr['created_at']);
            $updateAt = $this->convertDateTime($sales_order_arr['updated_at']);
//            $createdAt = str_replace('/', '-', $createdAt );
//            $createdAt = date("Y-m-d H:i", strtotime($createdAt));
//
//            $updateAt = $sales_order_arr['updated_at'];
//            $createdAt = str_replace('/', '-', $createdAt );
            $this->store_id = $sales_order_arr['store_id'];
            if (!$this->orderIdStatus($this->orderPrefix . $sales_order_arr['increment_id']))
                return 2;
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
...
//        Mage::getSingleton('core/resource_transaction');
            $order = $objectManager->create(\Magento\Sales\Model\Order::class)
                                   ->setIncrementId($this->orderPrefix . $sales_order_arr['increment_id'])
...
//            die();
            ...
            $billingAddress = $objectManager->create(\Magento\Sales\Model\Order\Address::class)
                //Mage::getModel('sales/order_address')
                                            ...
                                            ->setCompany(isset($sales_order_arr['billing_address']['company']) ? $sales_order_arr['billing_address']['company'] : "")
                                            ->setStreet($sales_order_arr['billing_address']['street'])
                                            ->setCity(isset($sales_order_arr['billing_address']['city']) ? $sales_order_arr['billing_address']['city'] : "")
                                            ->setCountryId(isset($sales_order_arr['billing_address']['country_id']) ? $sales_order_arr['billing_address']['country_id'] : "")
            ...
            if (!$sales_order_arr['is_virtual']) {
                $order->setShippingAddress($shippingAddress)
                      ...
                      ->setShippingDescription($sales_order_arr['shipping_description']);
            }
            $paymentMethod = 'checkmo';
            if (isset($sales_order_arr['payment']) && isset($sales_order_arr['payment']['method'])) {
                $paymentMethod = $sales_order_arr['payment']['method'];
            }
...
//                                          ->setMethod('checkmo')
            ...
            foreach ($sales_order_item_arr as $product) {
                $productOption = [];
                try {
                    if (isset($product['product_option']) && unserialize($product['product_option'])) {
                        $productOption = unserialize($product['product_option']);
                    }
                } catch (\Exception $e) {
                }
...
//                                           ->setProductOptions((isset($product['product_option']) && unserialize($product['product_option']))? unserialize($product['product_option']) : [])
                                           ->setQtyBackordered(null)
                                           ->setTotalQtyOrdered($product['qty_ordered'])
                                           ->setQtyOrdered($product['qty_ordered'])
                                           ->setQtyInvoiced($product['qty_invoiced'])
                                           ->setQtyShipped($product['qty_shipped'])
                                           ->setQtyRefunded($product['qty_refunded'])
                                           ->setQtyCanceled($product['qty_canceled'])
                ...
                if (isset($product['is_child']) && $product['is_child'] == 'yes')
                    $orderItem->setParentItemId($this->parent_id);
        
                elseif (isset($product['is_child']) && $product['is_child'] == 'no')
...
//            $transaction->addCommitCallback([$order, 'place']);
            ...
            try {
                if ($transaction->save()) {
                    $this->setLastOrderItemId();
                    $last_order_increment_id = $objectManager->create(\Magento\Sales\Model\Order::class)->getCollection()->getLastItem()->getIncrementId();
...
//                            return $this->setProcessing($sales_order_arr['order_state']);
//
...
//                            return $this->setPaymentReview();
                        ...
                        var_dump($this->orderPrefix . $sales_order_arr['increment_id'] . ' : ' . $e->getMessage());
                        echo "<pre>";
                        var_dump($sales_order_arr);
//                        $updateOrder = $this->getOrderModel($this->last_order_increment_id);
//                        $updateOrder->setState($sales_order_arr['order_state'])
//                              ->setStatus($sales_order_arr['order_status'])
//                              ->save();
//                        $updateOrder->unsetData();
                    }
...
//            var_dump($sales_order_arr);
//            echo "<pre>";
//            var_dump($sales_order_item_arr);
//            foreach ($sales_order_item_arr as $product) {
//                var_dump($product['product_option']);
//                var_dump(preg_replace('/[[:^print:]]/', '', $product['product_option']));
////                $product['product_option'] = preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", $product['product_option']);
//                var_dump(unserialize(preg_replace('/[[:^print:]]/', '', $product['product_option'])));
//            }
        }
    }
    
    public function setProcessing($orderStatus = '')
    {
        ...
        try {
            if ($this->setProcessing() == 1) {
                ...
                $order->setState(Mage_Sales_Model_Order::STATE_HOLDED, true)
                      ->setStatus(Mage_Sales_Model_Order::STATE_HOLDED, true)
                      ->save();
                $order->unsetData();
                return 1;
            }
        ...
        try {
            if ($this->setProcessing() == 1) {
                ...
                $order->setState(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW, true)
                      ->setStatus(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW, true)->save();
                $order->unsetData();
                return 1;
            }
        } catch (Exception $e) {
    ...
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        return $objectManager->create(\Magestore\MigrateData\Model\Order\Operations\Invoice::class);
    }
    
    public function getShipmentObj()
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        return $objectManager->create(\Magestore\MigrateData\Model\Order\Operations\Shipment::class);
    }
    
    public function getCreditmemoObj()
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        return $objectManager->create(\Magestore\MigrateData\Model\Order\Operations\Creditmemo::class);
    }
