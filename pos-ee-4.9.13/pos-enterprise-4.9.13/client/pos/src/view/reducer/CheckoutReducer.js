...
const initialState = {
    pages: [ProductList, Payment, GiftcardForm],
...
function index(state = initialState, action) {
    let eventData = {state, action};
    fire('reducer_checkout_index_before', eventData);
    state = eventData.state; // Rewrite state after event is called
    switch (action.type) {
