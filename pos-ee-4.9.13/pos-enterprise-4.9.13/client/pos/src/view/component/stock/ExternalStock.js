...
export class ExternalStock extends CoreComponent {
    ...
    componentWillReceiveProps(nextProps) {
        if (!nextProps.isShowExternalStock) {
            return;
        }
    ...
    backExternalStockPopup() {
        this.props.backExternalStockPopup();
        this.setState({
            product_id: null,
            locations: [],
            isLoading: false,
        ...
        return (
            <Fragment>
                <div className="modal-header">
                    <a className={canBack ? "back-product" : "hidden"} onClick={() => this.backExternalStockPopup()}>
                    </a>
                    <button type="button"
                ...
                <div className="modal-body" >
                    <div className={show_config} ref={this.setModalOptionElement}>
                        <div className="product-options-wrapper" >
                            {
                                productConfig ? (this.state.configOptions.length ? this.state.configOptions.map((option, index) => {
                                ...
                                }) : <div className="loader-product"></div>) : null
                            }
                        </div>
                    </div>
...
class ExternalStockContainer extends CoreContainer {
    ...
    static mapState(state) {
        let {isShowExternalStock} = state.core.product.viewProduct;
        let {stock_locations} = state.core.stock;
        return {stock_locations, isShowExternalStock};
    }
