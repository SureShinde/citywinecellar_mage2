...
export class ProductBundleViewComponent extends ProductAbstractViewComponent {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProduct && nextProduct.id && (!product || nextProduct.id !== product.id)) {
            ...
            if (nextProps.product.children_products && nextProps.product.children_products.length) {
    ...
    getStockChildrens(product) {
        ...
        if (!product.children_products) {
            return ProductService.getById(product.id).then(response => {
                Object.assign(product, response);
                this.getStockChildrens(product);
            });
        }
        let productIds = this.getChildrenProductIds(product);
        let request = StockService.getStockProducts(productIds);
        request.then(stocks => {
    ...
    hideNumpad() {
        if (this.state.numpadBundleOption) {
            ...
            this.validateOptionQty(productLink.product, currentQty, true);
            if (currentQty <= 0) {
    ...
    blurOptionQty(option) {
        ...
        if (productLink && productLink.product) {
            let product = productLink.product;
            if (!this.validateOptionQty(product, productLink.qty || 0, true)) {
     ...
     * @return {*}
     */
    validateOptionQty(product, qty, showError) {
        let bundleQty = this.state.bundleQty === '' ? 0 : parseFloat(this.state.bundleQty);
        ...
        if (!validateQty.success && showError) {
    ...
    validateBundleQty() {
        ...
        if (!bundleQty) {
            ...
            return {
                isValid: false,
                validQty: parseInt(bundleQty / bundleQtyIncrement, 10) - bundleQty % bundleQtyIncrement
            };
        }
        return {
            isValid: true
     ...
     */
    addToCart() {
        let validateBundleQty = this.validateBundleQty();
        if (!validateBundleQty.isValid) {
            return validateBundleQty.validQty ? this.updateBundleQty(validateBundleQty.validQty) : false;
        }
        ...
        this.state.bundleOptions.map(option => {
            ...
            validate =
                this.validateBundleOption(option, bundle_options, childrens, info_buyRequest, customOptions);
            validate.isValid = validate.isValid && oldValidate.isValid;

            if (
                validate.validQty
                && validate.validQty > oldValidate.validQty
            ) {
        ...
        if (!validate.isValid) {
            this.showError(this.props.t(validate.message));
            return validate.validQty ? this.updateBundleQty(validate.validQty) : false;
        }
    ...
    validateBundleOption(option, bundleOptions, childrens, infoBuyRequest, customOptions, checkRequire = true, checkQty = true) {
        ...
        if (option.product_links && option.product_links.length) {
            ...
            if (checkRequire && (!selectedProductLinks || !selectedProductLinks.length)) {
                return {
                    step: 'checkOptionRequire',
                    isValid: this.checkOptionRequire(option)
                };
            } else {
                selectedProductLinks.map(productLink => {
                    if (checkQty && ((productLink.qty && productLink.qty > 0) || option.required)) {
                        let validateOptionQty = this.validateOptionQty(productLink.product, productLink.qty || 0, false);
                        if (!validateOptionQty.success) {
                            result.step = 'validateOptionQty';
                            result.message = validateOptionQty.message;
                            result.isValid = false;
                            result.validQty =
                                Math.min(validateOptionQty.validQty, result.validQty || validateOptionQty.validQty);
                            return productLink;
                        }
                    }
