...
export class ProductConfigurableViewComponent extends ProductAbstractViewComponent {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.product && nextProps.product && this.isConfigurable(nextProps.product)) {
            if (!this.props.product || !this.props.product.id || this.props.product.id !== nextProps.product.id) {
                this.setState({current_qty: '', is_loading: false});
    ...
    getOptionsAndStockChildrens(product) {
        ...
        if (!product.search_string || !product.config_option) {
        ...
        request.then(response => {
            let configOptions = JSON.parse(JSON.stringify(response)).config_option;
            configOptions = this.recollectConfigOptionByProductStatus(
                configOptions, product.children_products
            );
     ...
     * @param configProduct
     */
    async validateConfigOptions(configProduct) {
        let validProductIds = [];
        ...
        let isChosenAllOptions = this.isChosenAllOptions();
        if (isChosenAllOptions && validProductIds && validProductIds.length === 1) {
            if (!configProduct.children_products) {
                configProduct.children_products = [];
            }
            if (!configProduct.children_products.find(product => validProductIds[0] === product.id)) {
                this.setState({load_child: true});
                let product = await ProductService.getById(validProductIds[0]);
                configProduct.children_products.push(product);
                if (product.id !== validProductIds[0]) {
                    return;
                }
                this.setState({load_child: false});
            }
        }
    ...
    getAvailableQty() {
        if (this.state.valid_product) {
            ...
            if (product.stocks && product.stocks.length) {
                ...
                qty = stock.is_in_stock ? stock.qty : 0;
            }
    ...
    isChosenAllRequireOptions() {
        ...
        if (this.props.product && this.props.product.custom_options) {
    ...
    getProductOptions() {
        ...
        return {
            ...
            shipment_type: AbstractProductTypeService.SHIPMENT_TOGETHER,
    ...
    template() {
        ...
        if ( this.state.valid_product && ModuleHelper.isAllowCheckExternalStock() ){
            ...
            <Fragment>
                <div className="modal-content" style={{
                    ...
                    <div className="modal-body" ref={this.setModalBodyElement}>
                        <div className="product-options-wrapper">
                            ...
                            }
                            {
                                this.props.product && (this.props.product.type_id === ProductTypeConstant.CONFIGURABLE)
                                    && undefined !== this.props.product.custom_options
                                    ?
                                    this.props.product.custom_options.map(option => {
                                        switch (option.type) {
                                            ...
                                            case OptionConstant.TYPE_DATE_TIME:
                                                return <DatePickerOption key={option.option_id}
                                                                         option={option}
                                                                         scrollToDateTime={
                                                                             this.scrollToDateTime.bind(this)
                                                                         }
                                                                         productPrice={this.props.product.price}
                                                                         addCustomOption={
                                                                             (optionValue) =>
                        ...
                        <div className={(this.state.load_child ? "loader-product" : "")}></div>
                        <div className={"product-info-price" + (this.state.load_child ? " hidden" : "")}>
                        ...
                        <div className={"product-avail-qty" + (this.state.load_child ? " hidden" : "")}>
                    ...
                    <div className="modal-bottom">
                        ...
                        <button type="button" ref={this.setAddToCartButtonElement}
                                className={
                                    "addtocart btn-default " + (!this.state.enableAddToCartButton ? "disabled" : "")
                                }
