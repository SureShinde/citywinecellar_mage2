...
class CreateCreditmemoStepPaymentOrderPaymentMethodComponent extends CoreComponent {
    ...
    getPaymentDescription(payment) {
        const { reference_number, card_type } = payment;
        if (!reference_number) {
            return null;
        }

        if (!card_type) {
            return  <span className="des">({reference_number})</span>;
        }

        return  <span className="des">({reference_number} - {card_type.toUpperCase()})</span>;
    }
