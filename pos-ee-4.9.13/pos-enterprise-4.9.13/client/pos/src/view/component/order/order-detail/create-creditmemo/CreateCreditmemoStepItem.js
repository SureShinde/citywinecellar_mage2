...
import ItemStorecreditComponent from "./step-item/item/CreateCreditmemoStepItemItemStorecredit";
...
class CreateCreditmemoStepItemComponent extends CoreComponent {
    ...
    getItemComponent(orderItem) {
        ...
        if (orderItem.product_type === ProductTypeConstant.STORE_CREDIT) {
            return <ItemStorecreditComponent {...props}/>;
        }
    ...
    template() {
        ...
        const { creditmemo_items_param } = this.props;
        let canRefundItemsChecker = [];
        if (Object.keys(creditmemo_items_param).length) {
            Object.keys(creditmemo_items_param).forEach(itemId => {
                let item = creditmemo_items_param[itemId];
                let orderItem = item.order_item;
                canRefundItemsChecker.push(OrderItemService.canRefundItemProductType(orderItem));
            });
        }

        /** check if all items cannot refund then we should disable use max qty */
        canRefundItemsChecker = [...new Set(canRefundItemsChecker)];
        let disableUseMaxQty = canRefundItemsChecker.length === 1 && !canRefundItemsChecker[0];
