...
export class OrderList extends AbstractGrid {
    ...
    componentWillMount() {
        ...
        if (Config.dataTypeMode && Config.dataTypeMode[SyncConstant.TYPE_ORDER]) {
            this.setState({mode: Config.dataTypeMode[SyncConstant.TYPE_ORDER]});
        }
        if (Config.session) {
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.isModeChange(nextProps)) {
            ...
            if ((nextProps.search_key === this.state.searchKey)
                ...
                && this.state.mode === nextProps.dataTypeMode[SyncConstant.TYPE_ORDER]
                ...
                if (parseFloat(nextProps.search_criteria.current_page) === 1) {
                    if (
                        this.state.changeModeReload
                        && typeof this.props.currentOrder !== "undefined"
                        && !nextProps.orders.find(item => item.increment_id === this.props.currentOrder.increment_id)
                    ) {
                        nextProps.orders.push(this.props.currentOrder);
                    }
                    ...
                    // }
                    if (!this.state.changeModeReload) {
                        this.selectOrder(nextProps.orders[0]);
                    }
                    this.setState({changeModeReload: false});
                    this.closeScanner();
                } else {
    ...
    isModeChange(nextProps) {
        if (
            nextProps.dataTypeMode
            && nextProps.dataTypeMode[SyncConstant.TYPE_ORDER]
            && (nextProps.dataTypeMode[SyncConstant.TYPE_ORDER] !== this.state.mode)
        ) {
            this.setState({
                mode: nextProps.dataTypeMode[SyncConstant.TYPE_ORDER],
                changeModeReload: true
            });
            this.startLoading();
            this.clearItems();
    ...
    selectOrder(order) {
        if(order && !order.items) {
            this.props.setIsDetailLoading(true);
            this.props.setCurrentOrder(order);
            OrderService.getByIncrement(order.increment_id).then((response) => {
                if (order.increment_id === response.increment_id) {
                    Object.assign(order, response);
                    if (this.props.currentOrder === order) {
                        this.props.setIsDetailLoading(false);
                        this.props.setCurrentOrder(Object.assign({}, order));
                    }
                }
            });
        } else {
    ...
    template() {
        ...
        return (
            <div className={"wrapper-order-left "
                ...
                <OrderSearchForm clickSearchBox={() => this.clickSearchBox()}
                                 blurSearchBox={() => this.blurSearchBox()}
    ...
    static mapDispatch(dispatch) {
        return {
            actions: {
                ...
                resetSyncActionUpdateOrder: () => dispatch(OrderAction.resetUpdatedOrdersList()),
