...
class OrderCreateCreditmemoComponent extends CoreComponent {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.order && nextProps.order.increment_id &&
            ...
            nextProps.order
            && nextProps.order.increment_id
            && this.props.order
            && this.props.order.increment_id
            && nextProps.order.increment_id === this.props.order.increment_id
            && nextProps.order.entity_id !== this.props.order.entity_id
        ) {
            this.updateCreditmemoAfterSyncOrder(nextProps.order);
        }
    }

    /**
     * Update credit memo after sync order
     *
     * @param updatedOrder
     */
    updateCreditmemoAfterSyncOrder(updatedOrder) {
        if (updatedOrder) {
            let creditmemo_items_param = this.state.creditmemo_items_param;
            Object.keys(creditmemo_items_param).forEach(itemId => {
                let creditmemoItem = creditmemo_items_param[itemId];
                let orderItem = updatedOrder.items.find(item =>
                    item.tmp_item_id && Number(item.tmp_item_id) === Number(itemId)
                );
                if (orderItem && orderItem.item_id !== itemId) {
                    creditmemoItem.order_item = orderItem;
                    creditmemo_items_param[orderItem.item_id] = creditmemoItem;
                    delete creditmemo_items_param[itemId];
                }
            });
            this.setState({creditmemo_items_param});
        }
    }
    ...
    async prepareCreditmemoParams(order) {
        if (order && order.items) {
            ...
            order.items.forEach(item => {
                if (OrderItemService.canRefundItemProductType(item)) {
                    productIds.push(item.product_id);
                }
            });
            let uniqueIds = {};
            productIds.forEach(productId => {
                uniqueIds[productId] = productId;
            });
            uniqueIds = Object.values(uniqueIds);
            let mode = this.props.mode;
            let stocks = await ProductService.getStockItemsToRefund(uniqueIds, mode);
            if (mode === SyncConstant.OFFLINE_MODE) {
                let gottenProductIds = stocks && Object.keys(stocks) ? Object.keys(stocks).map(Number) : [];
                let nonGetProductIds = uniqueIds.filter(id => !gottenProductIds.includes(id));
                if (nonGetProductIds && nonGetProductIds.length) {
                    let onlineStocks = await ProductService.getStockItemsToRefund(
                        nonGetProductIds,
                        SyncConstant.ONLINE_MODE
                    );
                    stocks = {...stocks, ...onlineStocks};
                }
            }
            ...
            order.items.forEach(item => {
                ...
                creditmemo_items_param[item.item_id] = {
                    ...
                    qty: OrderItemService.isDummy(item, order) ? undefined : 0,
                    ...
                    non_shipped_qty_to_return: OrderItemService.getNonShippedQtyToReturn(item, order),
    ...
    getCreditmemo() {
        ...
        if (this.state.step === CreateCreditmemoConstant.STEP_ITEM) {
            ...
            Object.keys(adjustments).forEach(key => {
                let isExtensionAttributes = false;

                let eventObject = {
                    key: key,
                    isExtensionAttributes: isExtensionAttributes
                };
                fire('refund_creditmemo_get_creditmemo_data_middle', eventObject);
                isExtensionAttributes = eventObject.isExtensionAttributes;

                if(isExtensionAttributes) {
                    if(typeof creditmemo.params.creditmemo.extension_attributes === 'undefined') {
                        creditmemo.params.creditmemo.extension_attributes = {};
                    }
                    creditmemo.params.creditmemo.extension_attributes[key] = adjustments[key];
                } else {
    ...
    useMaxQty(on = true) {
        ...
        Object.keys(creditmemo_items_param).forEach(itemId => {
            ...
            if (!OrderItemService.canRefundItemProductType(orderItem)) {
                return
            }
            let oldQty = item.qty;
    ...
    resetAdjustments(callback) {
        ...
        fire('component_order_create_creditmemo_reset_adjustments_after', {
            adjustments: adjustments,
            creditmemo: this.state.creditmemo,
