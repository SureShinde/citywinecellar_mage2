...
export class SessionList extends AbstractGrid {
    ...
    componentWillMount() {
        ...
        if (Config.dataTypeMode && Config.dataTypeMode[SyncConstant.TYPE_SESSION]) {
            this.setState({mode: Config.dataTypeMode[SyncConstant.TYPE_SESSION]});
        }
        if (Config.session) {
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.state.mode && this.props.dataTypeMode
            && nextProps.dataTypeMode && nextProps.dataTypeMode[SyncConstant.TYPE_SESSION]) {
            this.setState({mode: nextProps.dataTypeMode[SyncConstant.TYPE_SESSION]});
        }
        ...
        if (!this.isModeChange(nextProps)) {
            if (
                ...
                && this.state.mode === nextProps.dataTypeMode[SyncConstant.TYPE_SESSION]
    ...
    isModeChange(nextProps) {
        if (
            nextProps.dataTypeMode
            && nextProps.dataTypeMode[SyncConstant.TYPE_SESSION]
            && this.state.mode
            && (nextProps.dataTypeMode[SyncConstant.TYPE_SESSION] !== this.state.mode)
        ) {
            this.setState({mode: nextProps.dataTypeMode[SyncConstant.TYPE_SESSION]});
            this.startLoading();
            this.clearItems();
