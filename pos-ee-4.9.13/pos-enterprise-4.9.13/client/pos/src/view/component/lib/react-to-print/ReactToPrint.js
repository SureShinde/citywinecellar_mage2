...
class ReactToPrint extends Component {
    ...
    triggerPrint = (target) => {
        ...
        setTimeout(() => {
            PrinterService.printTarget(target);
    ...
    handlePrint = () => {
        ...
        const markLoaded = (linkNode, loaded) => {
            ...
            if (this.linksLoaded.length + this.linksErrored.length === this.linkTotal) {
                if (PrinterService.canUseApiPrint()) {
                    return this.callApiPrint(printWindow);
                }
    ...
    callApiPrint(printWindow) {
        const { onBeforePrint, onAfterPrint } = this.props;
        PrinterService.printCanvas(printWindow, {
            onBeforePrint,
            onAfterPrint,
        })
    }
