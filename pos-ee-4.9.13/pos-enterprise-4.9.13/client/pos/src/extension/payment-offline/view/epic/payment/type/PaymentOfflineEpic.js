...
import PaymentOfflineConstant from "../../../../constant/PaymentOfflineConstant";
...
export default function processPayment(action$) {
    return action$.ofType(PaymentConstant.START_PROCESS_SINGLE_PAYMENT)
        .mergeMap(action => {
            let payment_method = action.payment.method;

            if (payment_method.indexOf(PaymentOfflineConstant.PRE_FIX) !== 0)
