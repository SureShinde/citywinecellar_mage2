...
class CreateCreditmemoStepAdjustPointAdjustmentsComponent extends CoreComponent {
    ...
    prepareAdjustments(props) {
        ...
        order.extension_attributes.rewardpoints_earn && result.push({
            key  : CreateCreditmemoConstant.ADJUSTMENT_EARNED_KEY,
            label: t('Adjust Earned {{pointLabel}}', {
                pointLabel: order.extension_attributes.rewardpoints_earn > 1 ? pluralOfPointName : pointName
            })
        });
        order.extension_attributes.rewardpoints_spent && result.push({
            key  : CreateCreditmemoConstant.RETURN_SPENT_KEY,
            label: t('Return Spent {{pointLabel}}', {
                pointLabel: order.extension_attributes.rewardpoints_spent > 1 ? pluralOfPointName : pointName
            })
        });
        return result;
    }
