...
export class TyroPaymentService extends PaymentAbstract {
    static className = 'TyroPaymentService';
     ...
     * @return {Promise<*>}
     */
    async getResetUrl(payment = false) {
        payment = payment || await this.getPaymentMethod();
        if (payment.mode === TyroConstant.LIVE_MODE) {
            return TyroConstant.LIVE_RESET_URL;
        }
        if (payment.mode === TyroConstant.TEST_MODE) {
            return TyroConstant.TEST_RESET_URL;
        }
        return TyroConstant.SIMULATOR_RESET_URL;
    }
     ...
     * @return {Promise<{}>}
     */
    async resetTerminal(payment = false) {
        try {
           payment = payment || await this.getPaymentMethod();
           let resetUrl = await this.getResetUrl(payment);
           let mid = await this.getMerchantId(payment);
           let tid = this.getTerminalId();
           return await this.omc.get(`${resetUrl}terminals/${mid}/${tid}/reset`);
        } catch (e) {
           return {};
        }
    }
    ...
    async sendSaleRequest() {
        const payment = await this.getPaymentMethod();
        let iClient   = new TYRO.IClient(await this.getApiKey(payment), posProductInfo);
        await this.resetTerminal(payment);

        let countConfirmSignature = 0;
            ...
            iClient.customInitiatePurchase(
                ...
                },
                {
                    ...
                    questionCallback           : (question, answerCallback) => {
                        ...
                        if (countConfirmSignature > 0) {
                            return TyroPopupService.showPopup(this, {
                                message: question.text,
                                options,
                                isError,
                                answerCallback
                            }, iClient.cancelCurrentTransaction, true);
                        }

                        if (question.text === "APPROVED W/ SIGNATURE. Signature OK?" ) {
                            countConfirmSignature++;
                        }
