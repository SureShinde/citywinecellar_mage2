...
export class ProductListService extends CoreService {
    ...
    /**
     * Prepare item to update list product after delete product
     *
     * @param deleted_ids
     * @param items
     * @return {*}
     */
    prepareItemsToUpdateListAfterDeleteProduct(deleted_ids, items) {
        let result = false;
        if (deleted_ids && deleted_ids.length) {
            let deleteIds = {};
            deleted_ids.forEach(id => deleteIds[id] = id);
