...
export class QuoteTotalTaxService extends QuoteTotalCommonTaxCollectorService {
    ...
    collect(quote, address, total) {
        ...
        if ((isVirtual && AddressService.isBillingAddress(address)) ||
            ...
            // Add tax details for quote
            let applied_taxes = {}, process = 0;
            Object.keys(taxDetails.applied_taxes).forEach((taxId) => {
                let percent = taxDetails.applied_taxes[taxId].percent;
                if (!percent) {
                    return;
                }
                let taxRates = taxDetails.applied_taxes[taxId].rates;
                let amount = taxDetails.applied_taxes[taxId].amount;
                let baseAmount = baseTaxDetails.applied_taxes[taxId].amount;
                ...
                taxRateIds.forEach(taxRateId => {
                    let applied_tax = applied_taxes[taxRateId] || {
                        ...
                        code: taxRates[taxRateId].code,
                        title: taxRates[taxRateId].title,
                        percent: taxRates[taxRateId].percent,
                        process: process++,
                    };
                    let baseRealAmount, realAmount;
                    if (index > 1) {
                        realAmount = applied_tax.percent / percent * amount;
                        baseRealAmount = applied_tax.percent / percent * baseAmount;
                    } else {
                    ...
                    applied_tax.base_real_amount = CurrencyHelper.roundToFloat(
                        applied_tax.base_real_amount + baseRealAmount
                    );

                    applied_taxes[taxRateId] = applied_tax;
                    index--;
                });
            });
            quote.applied_taxes = applied_taxes;
