...
export class CreditmemoService extends AbstractOrderService {
    ...
    getAllPaymentType(type) {
        return Config.config[PaymentConstant.REFUND_PAYMENT_TYPE][type];
    }
     ...
     * @return {boolean}
     */
    isAcceptedPayment(method) {
        return this.getAllPaymentType(PaymentConstant.ACCEPTED_PAYMENTS).split(',').indexOf(method) !== -1;
    }
     ...
     * @return {boolean}
     */
    isUseTransactionPayment(method) {
        return this.getAllPaymentType(PaymentConstant.USE_TRANSACTION_PAYMENTS).split(',').indexOf(method) !== -1;
    }
            ...
            if (data.payments && data.payments.length) {
                data.payments.forEach(payment => {
                    if (payment.amount_paid > 0) {
                        ...
                        let payload = { payment, creditmemo, comment };
                        fire('service_creditmemo_create_creditmemo_before_add_comment', payload);
    ...
    initTotalCollectors() {
        if (!this.isInitTotalCollectors) {
            fire('service_creditmemo_init_total_collectors', {service: this});
